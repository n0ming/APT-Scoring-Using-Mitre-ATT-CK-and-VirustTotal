...
with open('Permission_Required_score.csv', mode='a', encoding='utf-8', newline='') as file:        
        ...
        for row in csv_reader:
            ...
            if row['Tactic_ID'] != 'TA0043' and row['Tactic_ID'] != 'TA0042':
                ...
                with open(f'{ID}.csv', 'r') as csvfile:
                    ...
                    for row in csv_reader:
                        print(row)
                        if row['Title'] == 'Permissions Required':
                            values = row['Value'].strip().split(',')
                            length = len(values)
                            count=10
                            print(values)
                            if length !=0:
                                count = score(values[0].strip())
                                if length ==1:
                                    count = score(values[0].strip())
                                else:
                                    for j in range(length):
                                        if count > score(values[j].strip()):
                                            count = score(values[j].strip())
                            print("dd",count)
                #print(md5)
                #print(count)
                ID= ID.replace("_", ".")
